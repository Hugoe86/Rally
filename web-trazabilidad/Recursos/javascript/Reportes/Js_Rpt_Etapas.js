...
function _load_vistas() {
    _launchComponent('vistas/Etapas/Principal.html', 'Principal');
}
...
function _eventos_principal() {
    try {
        ...
        $('#btn_exportar_pdf').on('click', function (e) {
            ...
            $table_pagination.bootstrapTable('tbl_reporte_etapas');
        });
        //  <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
        //  <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
function crear_tabla_reporte(posicion, arr_) {
    ...
    try {
        $('#tbl_reporte_etapas').bootstrapTable('destroy');

        $('#tbl_reporte_etapas').bootstrapTable({
            cache: false,
            striped: true,
            pagination: true,
            data: [],
...
function consultar_filtros() {
    ...
    try {
        ...
        if (valida_datos_requerido.Estatus) {
                ...
                success: function (datos2) {
                    if (datos2 !== null) {
                        ...
                        for (var cont_for = 0; cont_for < datos2.length ; cont_for++) {
                            arr_[cont_for] = datos2[cont_for].Numero;
                        }
            ...
            $.ajax({
                type: 'POST',
                url: 'controllers/EtapaDiaController.asmx/Consultar_Reporte',
                data: $data,
                dataType: "json",
                contentType: "application/json; charset=utf-8",
                async: false,
                ...
                success: function (datos) {
                    if (datos !== null) {
                        ...
                        $('#tbl_reporte_etapas').bootstrapTable('load', datos);
