...
function _load_vistas() {
    _launchComponent('vistas/Tiempos/Principal.html', 'Principal');
}
...
function _eventos_principal() {
    try {
        ...
        $('#btn_exportar_pdf').on('click', function (e) {
            ...
            var $table_pagination = $('#tbl_reporte_tiempos');
            $table_pagination.bootstrapTable('togglePagination');

            $table_pagination.tableExport({
                type: 'pdf',
                worksheetName: 'Etapas',
                fileName: 'Salidas_Evento[' + $('#cmb_evento_filtro :selected').text() + "]_Jornada[" + $('#cmb_jornada_filtro :selected').text() + "]",
                jspdf: {
                    orientation: 'l',
                    format: 'a3',
            ...
            $table_pagination.bootstrapTable('tbl_reporte_tiempos');
        });
        //  <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
        //  <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
function crear_tabla_reporte(posicion) {

    try {
        $('#tbl_reporte_tiempos').bootstrapTable('destroy');

        $('#tbl_reporte_tiempos').bootstrapTable({
            cache: false,
            striped: true,
            pagination: true,
            data: [],
...
function generateColumns(posicion) {
    var i;
    var columnas = [];
    var bol_ = true;
    ...
    for (i = 1; i <= posicion; i++) {
        ...
        columnas.push({ field: 'x' + i.toString(), title: '[' + i.toString() + ']', align: 'center', valign: 'top', visible: true, sortable: true });
    }
...
function consultar_filtros() {
    ...
    try {
        ...
        if (valida_datos_requerido.Estatus) {
            ...
            $.ajax({
                type: 'POST',
                url: 'controllers/EtapaDiaController.asmx/Consultar_Numero_Puntos_Control',
                data: $data,
                dataType: "json",
                contentType: "application/json; charset=utf-8",
                async: false,
                ...
                success: function (datos2) {
                    if (datos2 !== null) {
                        ...
                        crear_tabla_reporte(columnas_mostrar);
                    }
                }
            });
            ...
            $.ajax({
                type: 'POST',
                url: 'controllers/RptTiemposController.asmx/Consultar_Reporte',
                data: $data,
                dataType: "json",
                contentType: "application/json; charset=utf-8",
                async: false,
                ...
                success: function (datos) {
                    if (datos !== null) {
                        ...
                        $('#tbl_reporte_tiempos').bootstrapTable('load', datos);
