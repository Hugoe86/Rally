...
function _load_vistas() {
    _launchComponent('vistas/Participantes/Principal.html', 'Principal');
    _launchComponent('vistas/Participantes/Operacion.html', 'Operacion');
}
    ...
    try {
        ...
        $('#btn_agregar_documento').on('click', function (e) {
            ...
            if (valida_datos_requerido.Estatus) {
                ...
                var resultado = subir_archivo("#txt_ruta_imagen", "/rally/Participantes", "Temporales");
...

                {
...

                  {
                    ...
                    formatter: function (value, row) {
                        ...
                        var opciones = '<div style=" text-align: center;">';
                        opciones += '<div style="display:block"><a class="remove ml10 text-red" id="' + row.Documento_Id + '" href="javascript:void(0)" data-orden=\'' + JSON.stringify(row) + '\' onclick="btn_bloquear_click(this);" title="Bloquear"><i class="fa-user-times "></i>&nbsp;<span style="font-size:11px !important;">Bloquear</span></a></div>';
                        opciones += '</div>';
...
function btn_bloquear_click(tab) {
    var row = $(tab).data('orden');
...
function _validarDatos_Nuevo() {
    ...
    try {
        ...
        if ($('#cmb_sexo').val() == '' || $('#cmb_sexo').val() == undefined) {
            _output.Estatus = false;
            _output.Mensaje += '&nbsp;<i class="fa fa-angle-double-right"></i>&nbsp;El sexo.<br />';
        }
        //if ($('#txt_fecha_nacimiento').val() == '' || $('#txt_fecha_nacimiento').val() == undefined) {
        //    _output.Estatus = false;
