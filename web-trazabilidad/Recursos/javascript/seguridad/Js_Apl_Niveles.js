...
function _estado_inicial() {
    try {
        _habilitar_controles('Inicio');
        _limpiar_controles();
        $('#tbl_niveles').bootstrapTable('refresh', 'controllers/Niveles_Controller.asmx/Consultar_Niveles_Por_Filtros');
        _set_location_toolbar();
    } catch (e) {
        _mostrar_mensaje('Informe Técnico', e);
    }
...
function _cargar_tabla() {
    ...
    try {
        $('#tbl_niveles').bootstrapTable('destroy');
        $('#tbl_niveles').bootstrapTable({
            cache: false,
            width: 900,
            height: 400,
            striped: true,
            ...
            clickToSelect: true,
            columns: [
                { field: 'Nivel_ID', title: '', width: 0, align: 'center', valign: 'bottom', sortable: true, visible: false },
                { field: 'Nombre', title: 'Nombre', width: 500, align: 'left', valign: 'bottom', sortable: true, clickToSelect: false },
                {
                    ...
                    formatter: function (value, row) {
                        return '<div> ' +
                            '<a class="remove ml10 edit" id="' + row.Nivel_ID + '" href="javascript:void(0)" data-niveles=\'' + JSON.stringify(row) + '\' onclick="btn_editar_click(this);" title="Editar"><i class="glyphicon glyphicon-edit"></i></button>' +
                            '&nbsp;&nbsp;<a class="remove ml10 delete" id="' + row.Nivel_ID + '" href="javascript:void(0)" data-niveles=\'' + JSON.stringify(row) + '\' onclick="btn_eliminar_click(this);" title="Eliminar"><i class="glyphicon glyphicon-trash"></i></a>' +
                            '</div>';
                    }
                }
            ]
...
function _alta_niveles() {
    ...
    try {
        ...
        Niveles = new Object();
        Niveles.Nombre = $.trim($('#txt_nombre').val());

        var $data = JSON.stringify({ 'jsonObject': JSON.stringify(Niveles) });

        $.ajax({
            type: 'POST',
            url: 'controllers/Niveles_Controller.asmx/Alta',
            data: $data,
...
function _modificar_niveles() {
    var Niveles = null;
    var isComplete = false;

    try {
        Niveles = new Object();
        Niveles.Nivel_ID = parseInt($('#txt_nivel_id').val());
        Niveles.Nombre = $.trim($('#txt_nombre').val());
        ...
        $.ajax({
            type: 'POST',
            url: 'controllers/Niveles_Controller.asmx/Actualizar',
            data: $data,
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            async: false,
...
function _eliminar_niveles(nivel_id) {
    var Niveles = null;

    try {
        Niveles = new Object();
        Niveles.Nivel_ID = parseInt(nivel_id);

        var $data = JSON.stringify({ 'jsonObject': JSON.stringify(Niveles) });

        $.ajax({
            type: 'POST',
            url: 'controllers/Niveles_Controller.asmx/Eliminar',
            data: $data,
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            async: false,
...
function btn_editar_click(niveles) {
    var row = $(niveles).data('niveles');

    $('#txt_nivel_id').val(row.Nivel_ID);
...
function btn_eliminar_click(niveles) {
    var row = $(niveles).data('niveles');
    ...
    bootbox.confirm({
        ...
        callback: function (result) {
            if (result) {
                _eliminar_niveles(row.Nivel_ID);
            }
            _estado_inicial();
        }
    });
    ...
    try {
        show_loading_bar({
            ...
            finish: function (pct) {
                ...
                jQuery.ajax({
                    type: 'POST',
                    url: 'controllers/Niveles_Controller.asmx/Consultar_Niveles_Por_Filtros',
                    data: $data,
                    contentType: "application/json; charset=utf-8",
                    dataType: "json",
                    async: true,
                    ...
                    success: function (datos) {
                        if (datos !== null) {
                            $('#tbl_niveles').bootstrapTable('load', JSON.parse(datos.d));
                            hide_loading_bar();
                        }
                    }
                });
...
function _validate_fields(value, id, field) {
    ...
    try {
        Niveles = new Object();
        if (id !== null)
            Niveles.Nivel_ID = parseInt(id);

        switch (field) {
            case 'nombre':
                Niveles.Nombre = value;
                break;
            default:
        }
        ...
        $.ajax({
            type: 'POST',
            url: 'controllers/Niveles_Controller.asmx/Consultar_Niveles_Por_Nombre',
            data: $data,
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            async: false,
